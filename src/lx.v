Require Import ZProperty Confluence LambdaES.
...
Inductive sys_x : Rel pterm :=
  ...
  | reg_rule_varn : forall t n, term t -> sys_x (pterm_bvar (S n) [t]) (pterm_bvar n)
...
Proof.
  ...
  - unfold body in *.
    ...
    destruct H0.
    split.
    + apply term_sub with ((fv (pterm_fvar x)) \u fv (pterm_fvar x0)).
      * intros x' Hfv.
    ...
    + split.
      ...
      * apply term_bvar.
    + split.
      * apply term_sub with ((fv (pterm_fvar x)) \u fv (pterm_app t1 t2)).
        ** intros.
           unfold open.
           simpl.
           case x.
    ...
    + admit.
  - split.
    + apply term_sub with ((fv t1) \u (fv t2) \u (fv u)).
      * intros.
        unfold open.
        simpl.
        apply term_app.
